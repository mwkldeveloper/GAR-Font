...
from model.model_config import VQ_models,tokenizer_kwargs, style_args, gpt_models, gpt_kwargs, ffm_args
...
def main(args):
    ...
    # load pre-trained weights into Generator's content_encoder
    logger.info(f"Loading pre-trained weights into Generator's content_encoder")
    generator.content_encoder.load_state_dict(vq_model.encoder.state_dict())
    
    # freeze content_encoder parameters
    logger.info(f"Freezing Generator content_encoder parameters...")
    ...
    if args.generator_ckpt:
        ...
        start_epoch = 0
        del checkpoint
        logger.info(f"Resume training from checkpoint: {args.generator_ckpt}, train_steps={resume_train_steps}")
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for tar_img, content_ref, style_refs, attn_mask, valid in loader:
            ...
            with torch.no_grad():
                ...
                hw_ = hw
            vq_indices = (tar_vq_indices.reshape(B,-1))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default='generator_pre')
    parser.add_argument("--vq-model", type=str, choices=list(VQ_models.keys()), default="VQ-8")
    parser.add_argument("--vq-ckpt", type=str, required=True, help="ckpt path for VQ Tokenizer")
    parser.add_argument("--generator-ckpt", type=str, default=None, help="ckpt path for resume training")
    ...
    parser.add_argument("--val-every", type=int, default=5000, help="Run validation.")

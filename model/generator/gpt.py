...
def precompute_freqs_cis_1d(seq_len: int, n_elem: int, base: int = 10000, cls_token_num=120):
...
class ImgFeatureMapEmbedder(nn.Module):
    def __init__(self, in_channels, hidden_size, uncond_prob, token_num):
        super().__init__()
        self.img_feature_proj = MLP(in_features=in_channels, hidden_features=hidden_size, out_features=hidden_size)
        self.register_buffer("uncond_embedding", nn.Parameter(torch.randn(token_num, in_channels) / in_channels ** 0.5))
        self.uncond_prob = uncond_prob

    def token_drop(self, img_feature, force_drop_ids=None):
        if force_drop_ids is None:
            drop_ids = torch.rand(img_feature.shape[0], device=img_feature.device) < self.uncond_prob
        else:
            drop_ids = force_drop_ids == 1
        img_feature = torch.where(drop_ids[:, None, None], self.uncond_embedding, img_feature)
        return img_feature
...
class Transformer(nn.Module):
    def __init__(self, config: GPTModelArgs):
        ...
        self.img_feature_embedding = ImgFeatureMapEmbedder(config.img_feature_channel, config.dim, config.feature_dropout_prob,config.img_feature_code_len) # (bs, C, H, W) => (bs, (seq_len)[H*W], dim)
        ...
        self.freqs_cis = precompute_freqs_cis_1d(self.target_token_len, self.config.dim // self.config.n_head, self.config.rope_base, self.img_feature_code_len)
    ...
    def setup_caches(self, max_batch_size, max_seq_length, dtype,device):
        ...
        for b in self.layers:
            b.attention.kv_cache = KVCache(max_batch_size, max_seq_length, self.config.n_head, head_dim, dtype,device)
    ...
    def forward(
        ...
        **kwargs
        ...
        if idx is not None and imgs_feature_map is not None:
            # for model training
            img_embeddings = self.img_feature_embedding(imgs_feature_map,train=self.training)
            ...
            if imgs_feature_map is not None:
                ...
                token_embeddings = self.img_feature_embedding(imgs_feature_map,train=self.training)
            else:
                # idx is not None
        ...
        if valid is not None:
            loss_all = F.cross_entropy(logits.contiguous().view(-1, logits.size(-1)), targets.contiguous().view(-1), reduction='none')
